...
export async function exportAssessmentGrade(data: {academicClass: AcademicClass, khsDetails: KhsDetail[]}) {
  ...
  data?.khsDetails.forEach((items: KhsDetail, i: number) => {
    const rowdata: (string | number)[] = [
      ...
      ...assessmentDetails.map((assessmentDetail: AssessmentDetail) => {
        const grade = items.khsGrade.find((g: KhsGrade) => g.assessmentDetailId === assessmentDetail.id);
