...
async function main() {
      ...
      data: {
        ...
        lecturer: {
          create: {
            ...
            frontTitle: (i <= 2 ? "Dr." : ""),
            backTitle: (i <= 2 ? "S.Pd., M.Sc" : "M.Kom"),
            degree: (i <= 2 ? DegreeStatus.S3 : DegreeStatus.S2),
  ...
  for (let i = 1; i <= 6; i++) {
    students.push({
      ...
      gender: (i <= 3 ? Gender.PRIA : Gender.WANITA),
      majorId: (i % 2 === 0 ? 1 : 2),
      statusRegister: StatusRegister.BARU,
      lecturerId: (i % 2 === 0 ? lecturerSI[0].id : lecturerTI[0].id),
    })
  };
  // for (let i = 0; i <= 9; i++) {
  //   students.push({
